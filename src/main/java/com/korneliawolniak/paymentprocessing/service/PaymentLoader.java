...
public class PaymentLoader {
  ...
  public Payment load(PaymentInput paymentInput) {
    ...
    Debtor debtor = new Debtor(paymentInput.debtor().name(), paymentInput.debtor().accountNumber());
  ...
  private Transaction mapTransaction(TransactionInput transactionInput, UUID paymentId) {
    Creditor creditor =
        new Creditor(
            transactionInput.creditor().name(), transactionInput.creditor().accountNumber());

    return new Transaction(UUID.randomUUID(), paymentId, creditor, transactionInput.amount());
  }
}
